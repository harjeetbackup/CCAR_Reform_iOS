...
class DateConverterVC: UIViewController, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    func loaddata()
    {
        ...
        if strL == "English"
        {
            guard let strDate =  Int(strD),
                let strYear = Int(strY),
                let strMonth = strM else {return}
                ...
                case .success(_):
                    if response.result.value != nil{
                        ...
                        if let dic = response.result.value as? [String : Any]{
                            if  let strDate = dic["gd"] ,
                                let strMonth =  dic["gm"] ,
                                let strYear = dic["gy"]  {
                                self.lblSDayValue?.text = "\(strDate)"
                                self.lblSMonthValue?.text = "\(strMonth)"
                                self.lblSYearValue?.text = "\(strYear)"
                                self.strY = "\(strYear)"
                                self.strM = "\(strMonth)"
                            }
                        }
